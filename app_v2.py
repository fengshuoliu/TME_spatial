...
def init_state() -> None:
    ...
    if "local_folder_browser_choice" not in st.session_state:
        st.session_state["local_folder_browser_choice"] = ""
...
def list_directory_suggestions(raw_path: str, app_root: Path) -> tuple[Path, List[Path], List[Path], str | None]:
    try:
        resolved = resolve_folder(raw_path or ".", app_root)
    except Exception as exc:
        return app_root, [], [], str(exc)

    if resolved.is_file():
        resolved = resolved.parent

    if not resolved.exists() or not resolved.is_dir():
        return resolved, [], [], f"Folder not found: {resolved}"

    subdirs = sorted([p for p in resolved.iterdir() if p.is_dir()], key=lambda p: p.name.lower())
    data_files = sorted(
        [p for p in resolved.iterdir() if p.is_file() and p.suffix.lower() in {".csv", ".txt"}],
        key=lambda p: p.name.lower(),
    )
    return resolved, subdirs, data_files, None


def render_local_folder_selector(app_root: Path) -> tuple[str, List[str]]:
    st.text_input(
        "Folder path (absolute or relative to the repo root)",
        key="local_folder_input",
        placeholder="Example: data/demo1 or /Users/you/project/data/demo1",
    )

    current_path = st.session_state.get("local_folder_input", "")
    resolved_dir, subdirs, data_files, error_message = list_directory_suggestions(current_path, app_root)
    ...
    st.caption(f"Current directory: `{resolved_dir}`")

    controls = st.columns([1, 1, 6])
    with controls[0]:
        if st.button("Go up", key="go_up_local_folder"):
            parent = resolved_dir.parent if resolved_dir != resolved_dir.parent else resolved_dir
            try:
                display_parent = str(parent.relative_to(app_root))
                st.session_state["local_folder_input"] = display_parent if display_parent else "."
            except Exception:
                st.session_state["local_folder_input"] = str(parent)
            st.session_state["local_folder_browser_choice"] = ""
            st.rerun()
    with controls[1]:
        if st.button("Refresh", key="refresh_local_folder"):
            st.rerun()

    suggestion_options = [""]
    suggestion_labels = {"": "Select a subfolder to jump into it"}
    for p in subdirs:
        option_value = str(p)
        suggestion_options.append(option_value)
        suggestion_labels[option_value] = p.name

    selected_subdir = st.selectbox(
        "Available subfolders",
        options=suggestion_options,
        index=0,
        key="local_folder_browser_choice",
        format_func=lambda x: suggestion_labels.get(x, x),
        help="This acts like a simple directory suggestion widget under the text box.",
    )

    if selected_subdir:
        chosen = Path(selected_subdir)
        try:
            display_child = str(chosen.relative_to(app_root))
            st.session_state["local_folder_input"] = display_child if display_child else "."
        except Exception:
            st.session_state["local_folder_input"] = str(chosen)
        st.session_state["local_folder_browser_choice"] = ""
        st.rerun()

    if data_files:
        st.success(f"Found {len(data_files)} CSV/TXT files in this folder.")
        with st.expander("Preview files in current folder", expanded=False):
            preview_names = [p.name for p in data_files[:100]]
            if len(data_files) > 100:
                st.caption(f"Showing first 100 of {len(data_files)} files.")
    else:
        st.info("No CSV/TXT files found in the current folder yet.")

    return str(resolved_dir), [str(p) for p in data_files]
